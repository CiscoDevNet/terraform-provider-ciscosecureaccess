...
resource "ciscosecureaccess_destination_list" "content_dl" {
  name = "TF Content Dest List"
  destinations = [
    {
      comment     = "Content filtered destination managed by TF"
      type        = "ipv4"
      destination = "11.22.33.0/24"
    }
  ]
}
...
resource "ciscosecureaccess_access_policy" "user_to_default_content_categories" {
  name         = "terraform-default-content-category-policy"
  action       = "block"
  enabled      = true
  log_level    = "LOG_ALL"
  traffic_type = "PUBLIC_INTERNET"
  source_ids   = [data.ciscosecureaccess_identity.identity.identities[0].id]
  destination_list_ids = [resource.ciscosecureaccess_destination_list.content_dl.id]
  content_category_list_ids = [
    data.ciscosecureaccess_content_category_list.ad_content_category_list.content_category_lists[0].id
  ]
  description = "Allow all traffic to the organization default content category list"
}
...
data "ciscosecureaccess_content_category_list" "ad_content_category_list" {
  filter = "ads"
}
